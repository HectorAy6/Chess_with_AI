...
void Peon::obtener_movimientos_posibles(std::set<Pos> &result, Pos p, Tablero *t){
    result.clear();
    int factor = color==NEGRA?-1:1;
    if(!t->ocupado(Pos(p.x-1*factor, p.y))) {
        result.insert(Pos(p.x-1*factor, p.y));
        if(primer_movimiento and !t->ocupado(Pos(p.x-1*factor, p.y)) )result.insert(Pos(p.x-2*factor, p.y));
    }
